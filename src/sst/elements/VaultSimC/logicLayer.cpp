  ...
  Component( id ), memOps(0)
{
    ...
    llID = params.find<unsigned int>("llID", found);
    if (!found) {
        dbg.fatal(CALL_INFO, -1, "no 'llID' defined for logiclayer\n");
    }

    bwlimit = params.find<unsigned int>( "bwlimit", found);
    if (!found) {
        dbg.fatal(CALL_INFO, -1, "no 'bwlimit' defined for logiclayer\n");
    }

    VAULT_SHIFT = params.find<uint64_t>( "VAULT_SHIFT", 7 );

    LL_SHIFT = params.find<uint64_t>( "LL_SHIFT", 12 );

    LL_MASK = params.find<uint64_t>( "LL_MASK", found );
    if (!found) {
        dbg.fatal(CALL_INFO, -1, "no 'LL_MASK' defined for logiclayer\n");
    }

    bool terminal = params.find<bool>("terminal", false);

    int numVaults = params.find<int>("vaults", -1);
    if ( -1 != numVaults) {
        ...
        output->output(" Connected %d Vaults\n", numVaults);
    } else {
        dbg.fatal(CALL_INFO, -1, " no 'vaults' defined for LogicLayer\n");
    }
...
{
  output->output("Logic Layer %d completed %lld ops\n", llID, memOps);
  return 0;
}
...
void logicLayer::init(unsigned int phase) {
    ...
    if ( !phase ) {
        toCPU->sendInitData(new SST::Interfaces::StringEvent("SST::VaultSim::MemReqEvent"));
    }
