...
pagedMultiMemory2::pagedMultiMemory2(Component *comp, Params &params) : SimpleMemBackend(comp, params), pagesInFast(0), lastMin(0) {
    ...
    self_link = comp->configureSelfLink("Self", access,
                                        new Event::Handler<pagedMultiMemory2>(this, &pagedMultiMemory2::handleSelfEvent));
    ...
    comp->registerClock(clock_freq, 
                        new Clock::Handler<pagedMultiMemory2>(this, 
                                                             &pagedMultiMemory2::quantaClock));
    ...
    {
        // use our own callbacks for nv or cube link
        std::string access_time = "1ns";
        slow_link = comp->configureLink( "cube_link", access_time,
                                        new Event::Handler<pagedMultiMemory2>(this, &pagedMultiMemory2::handleMemCompReq));
    }
}

// should we add it?
bool pagedMultiMemory2::checkAdd(pageInfo &page) {
    // only add if the dram isn't too busy
    if (dramBackpressure && slowQ.size() >= 4) return false;
...
void pagedMultiMemory2::do_FIFO_LRU( pageInfo &page, bool &inFast, bool &swapping) {  
    swapping = 0; 
    if (0 == page.inFast) {
        // not in fast
...
void pagedMultiMemory2::do_LFU( Addr addr, pageInfo &page, bool &inFast, bool &swapping) {
...
bool pagedMultiMemory2::issueRequest(ReqId id, Addr addr, bool isWrite, unsigned numBytes ){
...
void pagedMultiMemory2::clock(){
    // put things in the slow memory 
    while (!slowQ.empty()) {
        Req *req = slowQ.front();
...
void pagedMultiMemory2::printAccStats() {
  FILE * pFile;
  char buf[100];
...
void pagedMultiMemory2::handleSelfEvent(SST::Event *event){
    ...
    if (si != swapToSlow_Reads.end()) {
        ...
        delete ev;
    } else if (si_w != swapToFast_Writes.end()) {
...
bool pagedMultiMemory2::quantaClock(SST::Cycle_t _cycle) {
    if (collectStats) printAccStats();
...
void pagedMultiMemory2::moveToFast(pageInfo &page) {
...
void pagedMultiMemory2::moveToSlow(pageInfo *page) {
    assert(page->swapDir == pageInfo::NONE);
...
void pagedMultiMemory2::handleMemCompReq(SST::Event *event){
    ...
    if (ev) {
        ...
        // events may return out of order

        //Req* req = reqs.front();
        std::list<Req*>::iterator reqI 
...
#warning we should actually generate proper ids
        if (id != 0) {
        ...
        if (si != swapToSlow_Writes.end()) {
            // this is a returning write from the slow
            ...
            delete req;
        } else if (si_r != swapToFast_Reads.end()) {
...
void pagedMultiMemory2::swapDone(pageInfo *page, const uint64_t addr) {
    const uint64_t pageAddr = addr >> pageShift;
...
bool pagedMultiMemory2::pageIsSwapping(const pageInfo &page) {
