...
#ifndef _H_SST_MEMH_PAGEDMULTI2_BACKEND
#define _H_SST_MEMH_PAGEDMULTI2_BACKEND
...
class pagedMultiMemory2 : public SimpleMemBackend {
public:
    pagedMultiMemory2(Component *comp, Params &params);
...
private:
    ...
    struct Req : public SST::Core::Serialization::serializable {
	  ...
	  private:
        Req() {}
		ImplementSerializable(SST::MemHierarchy::pagedMultiMemory2::Req)
    };
    p2Info::pageInfo::pageList_t pageList; // used in FIFO
    ...
    bool checkAdd(p2Info::pageInfo &page);
    void do_FIFO_LRU( p2Info::pageInfo &page, bool &inFast, bool &swapping);
    void do_LFU( Addr, p2Info::pageInfo &page, bool &inFast, bool &swapping);
    
    void printAccStats();
    // queue up requests before sending to the slow memory
    queue<Req *> slowQ;
...
public:    
    class MemCtrlEvent;
private:    
    typedef map<MemCtrlEvent *, p2Info::pageInfo*> evToPage_t;
    typedef map<Req *, p2Info::pageInfo*> reqToPage_t;
    ...
    // tracks 
    std::set<ReqId> outToSlow;
    std::map<uint64_t, std::list<Req*> > slowReqs;

    void handleMemCompReq(SST::Event *event);
    void swapDone(p2Info::pageInfo *, uint64_t);
    void moveToFast(p2Info::pageInfo &);
    void moveToSlow(p2Info::pageInfo *);
    bool pageIsSwapping(const p2Info::pageInfo &page);
...
public:    
    ...
    public:
        ...
        ImplementSerializable(SST::MemHierarchy::pagedMultiMemory2::MemCtrlEvent);     
    };

    typedef map<uint64_t, p2Info::pageInfo> pageMap_t;
    pageMap_t pageMap;
    uint maxFastPages;
    uint pageShift;
    uint pagesInFast;
